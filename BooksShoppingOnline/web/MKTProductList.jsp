...
<html lang="en">
    <head>
        ...
        <title>Danh sách sản phẩm - MKT</title>
                    ...
                    <div style="margin: 2%;">
                        <div class="row">
                            <div class="col-md-2 text-center bg-warning" style="border-radius: 10px">
                                <b style="font-size: 15px"><span style="color: red">Tổng số sản phẩm: ${total}</span> <br> <span style="color: green">Số sản phẩm hiển thị: ${PublishedProduct}</span></b>
                            </div>
                            ...
                            <div class="col-md-2 text-center">
                                <select class="dropdown-font-new" aria-label="Default select example" onchange="location = this.value;">
                                    ...
                                    <option value="marketingproductlist?${historyKey}${historyCategoryId}${historyStatus}&value=sale_prices" ${value eq "sale_prices" ? "Selected" : ""}>
                                        Giá sale tăng dần
                                    </option>
                                    <option value="marketingproductlist?${historyKey}${historyCategoryId}${historyStatus}&value=sale_prices&type=desc" ${value eq "sale_prices" && type eq "desc" ? "Selected" : ""}>
                                        Giá sale giảm dần
                                    </option> 
                                    <option value="marketingproductlist?${historyKey}${historyCategoryId}${historyStatus}&value=product_name" ${value eq "product_name" ? "Selected" : ""}>
                                        Tiêu đề tăng dần
                                    </option> 
                                    <option value="marketingproductlist?${historyKey}${historyCategoryId}${historyStatus}&value=product_name&type=desc" ${value eq "product_name" && type eq "desc" ? "Selected" : ""}>
                                        Tiêu đề giảm dần
                                    </option> 
                                    <option value="marketingproductlist?${historyKey}${historyCategoryId}${historyStatus}&value=category_id" ${value eq "category_id" ? "Selected" : ""}>
                                        Thể loại tăng dần
                                    </option> 
                                    <option value="marketingproductlist?${historyKey}${historyCategoryId}${historyStatus}&value=category_id&type=desc" ${value eq "category_id" && type eq "desc" ? "Selected" : ""}>
                                        Thể loại giảm dần
                                    </option> 
                            ...
                            <div class="col-md-2 text-center">
                                <a href="MKTAddProduct.jsp"><button type="button" class="btn btn-danger " style="">Thêm sản phẩm</button></a>
                            </div>
                            <div class="container rounded bg-white mt-5 mb-5" >

                            <div class="col-md-3 text-center">
                                <select class="dropdown-font-new" aria-label="Default select example" onchange="location = this.value;">
                                    <option value="marketingproductlist?${historyKey}${historyCategoryId}${historyStatus}${historyValue}${historyType}&page_size=6" ${page_size == 6 ? "Selected" : ""}>
                                       Hiển thị số bản ghi 
                                    </option>
                                    <option value="marketingproductlist?${historyKey}${historyCategoryId}${historyStatus}${historyValue}${historyType}&page_size=10" ${page_size == 10 ? "Selected" : ""}>
                                        10
                                    </option> 
                                    <option value="marketingproductlist?${historyKey}${historyCategoryId}${historyStatus}${historyValue}${historyType}&page_size=25" ${page_size == 25? "Selected" : ""}>
                                        25
                                    </option>
                                    <<option value="marketingproductlist?${historyKey}${historyCategoryId}${historyStatus}${historyValue}${historyType}&page_size=50" ${page_size == 50 ? "Selected" : ""}>
                                        50
                                    </option> 
                                </select>
                            </div>
                                <table class="table text-center" id="sortTable">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Mã sản phẩm</th>
                                            <th><a href="#" style="text-decoration: none">Ảnh sản phẩm</a></th>
